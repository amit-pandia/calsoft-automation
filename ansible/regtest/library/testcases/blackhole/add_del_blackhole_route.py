...
def add_del_blackhole(module):
    ...
    if subnet_mask == "24":
        blackhole_ip = '10.0.{}.0/24'.format(eth_list[0])
    elif subnet_mask == "32":
        blackhole_ip = '10.0.{0}.{1}/32'.format(eth_list[0], p_list[0][-2:])

    if module.params['delete']:
            execute_commands(module, 'ip route delete {}'.format(blackhole_ip))
            
    else:
            execute_commands(module, 'ip route add blackhole {}'.format(blackhole_ip))
